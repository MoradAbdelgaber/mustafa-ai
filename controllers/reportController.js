...
function calculateAttendanceMetrics({
  ...
  let delayMinutes = 0, earlyExitMinutes = 0, overtimeMinutes = 0;
  ...
  if (actualCheckIn && lastEntryPreventionUTC && actualCheckIn > lastEntryPreventionUTC) {
    actualCheckIn = null; // تهمل
  }
  ...
  if (!daySchedule.overtime_eligible && actualCheckIn) {
    ...
    if (actualCheckIn < dayStartUTC && (!workRegStartUTC || actualCheckIn >= workRegStartUTC)) {
  ...
  if (daySchedule.overtime_eligible) {
    ...
    if (otHours > 0) {
      ...
      if (daySchedule.minimum_working_hours_overtime &&
  ...
  if (actualCheckIn && actualCheckIn > dayStartUTC) {
    ...
    if (daySchedule.allowed_delay_minutes && dm <= daySchedule.allowed_delay_minutes) {
      dm = 0;
    }
    dmins = dm;
  }
  ...
  if (actualCheckOut && actualCheckOut < dayEndUTC && finalWorkHours < (daySchedule.official_working_hours || 8)) {
    let diffE = (dayEndUTC - actualCheckOut) / 60000;
    let e2 = Math.round(diffE);
    if (daySchedule.allowed_exit_minutes && e2 <= daySchedule.allowed_exit_minutes) {
      e2 = 0;
    }
    emins = e2;
  }
  ...
  else if ((finalWorkHours + totalRest) < (daySchedule.official_working_hours || 8)) {
...
exports.getFullAttendanceReport = async (req, res) => {
  try {
    ...
    .find({ ...empMatch, owner: req.userId })
    ...
    if (+show_official_holidays === 1) {
      officialHolidays = await OfficialHoliday.find({
        holiday_date: { $gte: startDate, $lte: endDate },owner: req.userId
      }).lean();
    }
    ...
    for (const emp of employees) {
      for (const dateObj of allDates) {
        ...
        let record = {
          ...
          minimum_working_hours_overtime: (daySchedule ?.minimum_working_hours_overtime) || 10,
        ...
        if (daySchedule.work_registration_start_time) {
          wrStartUTC = localTimeToUTC(dateObj, daySchedule.work_registration_start_time, timeZone);
        }

        // overtimeStartUTC
        let overtimeStartUTC = null;
        if (daySchedule.overtimeStart) {
          overtimeStartUTC = localTimeToUTC(dateObj, daySchedule.overtimeStart, timeZone);
        ...
        if (daySchedule.last_entry_prevention_time) {
          lastEntryPreventionUTC = localTimeToUTC(dateObj, daySchedule.last_entry_prevention_time, timeZone);
          daySchedule.lastEntryPreventionUTC = lastEntryPreventionUTC;
        }

        // first_exit_allowed_time => UTC
        let firstExitUTC = null, lastExitUTC = null;
        if (daySchedule.first_exit_allowed_time) {
          firstExitUTC = localTimeToUTC(dateObj, daySchedule.first_exit_allowed_time, timeZone);
        }
        if (daySchedule.last_exit_allowed_time) {
          lastExitUTC = localTimeToUTC(dateObj, daySchedule.last_exit_allowed_time, timeZone);
        }

        // (التعديل الجديد) إضافة الـ offset على الأوقات الأخرى إذا وجد endDayOffset:
        if (offset > 0) {
          if (lastEntryPreventionUTC) {
            lastEntryPreventionUTC = new Date(lastEntryPreventionUTC.getTime() + offset * 24 * 3600 * 1000);
          }

          if (offset > 0 && overtimeStartUTC) {
            overtimeStartUTC = new Date(overtimeStartUTC.getTime() + offset * 24 * 3600 * 1000);
            daySchedule.overtimeStartUTC = overtimeStartUTC;
          }
          if (firstExitUTC) {
            firstExitUTC = new Date(firstExitUTC.getTime() + offset * 24 * 3600 * 1000);
          }
          if (lastExitUTC) {
            lastExitUTC = new Date(lastExitUTC.getTime() + offset * 24 * 3600 * 1000);
          }
        }
        // نهاية التعديل

        // فلترة البصمات
        let dayFingerLogs = fingerLogs.filter(f => {
        ...
        // إجازة يومية
        let foundVacation = vacations.find(v =>
        ...
        if (holidayDoc) {
          ...
          record.status_code = metrics.statusCode;
          record.work_hours = +metrics.workHours.toFixed(2);
          record.check_in = metrics.finalCheckIn ? utcToLocalString(metrics.finalCheckIn, timeZone) : null;
          record.check_out = metrics.finalCheckOut ? utcToLocalString(metrics.finalCheckOut, timeZone) : null;
          record.base_work_hours = +metrics.baseWork.toFixed(2);
        ...
        if (daySchedule.works_for_daily_wage &&
          !['holiday', 'annual_leave', 'sick_leave', 'other_leave', 'absent', 'rest_day'].includes(record.status_code)
        ) {
          record.salary_earned_for_work = daySchedule.daily_salary || 0;
        } else if (!daySchedule.works_for_daily_wage &&
          !['holiday', 'annual_leave', 'sick_leave', 'other_leave', 'absent', 'rest_day'].includes(record.status_code)
        ) {
          record.salary_earned_for_work = record.work_hours * (daySchedule.hourPrice || 0);
        }
        ...
        if (record.early_exit_minutes > 0) {
          ...
          record.early_exit_deduction = record.early_exit_minutes * earlyExitMinPrice;
        }
        ...
        if (!holidayDoc && daySchedule.startTime !== '00:00:00' && daySchedule.endTime !== '00:00:00') {
    ...
    for (let r of finalResults) {
      ...
      let stDoc = await StatusName.findOne({ status_code: r.status_code }).lean();
      if (stDoc) {
    ...
    // ملخص
    let sumDelay = 0, sumEarly = 0, sumOT = 0, sumAbs = 0, sumRewards = 0, sumPenalties = 0, sumNet = 0;
