...
exports.getAllVacations = async (req, res) => {
  try {
    ...
    const data = await Vacation.find(query);
    res.json(data);
  } catch (error) {
    console.error(error);
...
exports.getVacationById = async (req, res) => {
  try {
    ...
    if (!vac) {
      return res.status(404).json({ message: 'Vacation not found or not owned by you' });
    }
    res.json(vac);
  } catch (error) {
    console.error(error);
...
exports.deleteVacation = async (req, res) => {
  try {
    ...
    const deleted = await Vacation.findOneAndDelete({ _id: id, owner: req.userId });
    if (!deleted) {
      return res.status(404).json({ message: 'Vacation not found or not owned by you' });
    }
